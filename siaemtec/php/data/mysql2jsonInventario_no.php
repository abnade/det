<?php 																												
$sql="SELECT ID_EQUIPO_INVENTARIO,(certificado(NO_CONTROL)+constancia(NO_CONTROL) + musuario(NO_CONTROL) + mservicio(NO_CONTROL)) AS docst , NO_CONTROL, ID_EQUIPO, EQUIPO, MARCA, OBSOLESCENCIA, MODELO, SERIE, NO_INVENTARIO, SERVICIO, AREA AS UBICACION, CUERPO, EXTENSION, ID_PROVEEDOR, PROVEEDOR, FECHA_DE_INSTALACION, ESTADO, PROPIEDAD, PRECIO, FECHA_CREACION, GARANTIA, COMPRA_POR, CONVENIO_EVENTO, PARTIDA, NOTAS, YEAR(CURDATE())-YEAR(FECHA_COMPRA) AS ANTIGUEDAD, FECHA_COMPRA, VIDA_UTIL, FECHA_BAJA, NO_ORDEN, ESTADO_FUNCIONAL AS EDO_ORDEN, case FUERA_SERVICIO when 'SI' then 'NO FUNCIONA' when 'NO' then 'FUNCIONA' when 'PARCIALMENTE' then 'PARCIALMENTE' else 'NO DISPONIBLE' end as EDO_FUNCIONAL_EQUIPO, case FUERA_SERVICIO when 'SI' then '1' when 'NO' then '2' when 'PARCIALMENTE' then '3' else '0' end as EDO_F FROM inventariovw1 WHERE ESTADO ='ACTIVO' AND OBSOLESCENCIA=0 OR OBSOLESCENCIA IS NULL ORDER BY `docst` DESC ";
...
= 'SIN ORDENES'; } 
        ...
        if($row['OBSOLESCENCIA']==1){
            $row['NO_CONTROL']='<div class="sr-text">'.$row['NO_CONTROL'].'</div>';
        }else if ($row['OBSOLESCENCIA']==3)
        { $row['NO_CONTROL']='<div class="o-text">'.$row['NO_CONTROL'].'</div>';
           }else if ($row['OBSOLESCENCIA']==2)
        { $row['NO_CONTROL']='<div class="rc-text">'.$row['NO_CONTROL'].'</div>';
        
        }else{
            $row['NO_CONTROL']='<div class="no-text">'.$row['NO_CONTROL'].'</div>';
        }
...
$row['VER_EDITAR']='<a href="modificarEquipo.html?idEquipo='.$row['ID_EQUIPO_INVENTARIO'].'" class="btn btn-sm btn-outline-info">VER/EDITAR</a>';
