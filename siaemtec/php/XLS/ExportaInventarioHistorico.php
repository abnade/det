       ...
       // echo "id: " . $row["ID_EQUIPO_INVENTARIO"]. "<br>";
		$objPHPExcel->setActiveSheetIndex(0)
           ...
           // ->setCellValue('C'.$n,$row["ID_EQUIPO"])
			...
			->setCellValue('D'.$n,strtoupper($row["MARCA"]))
			->setCellValue('E'.$n,strtoupper($row["MODELO"]))
            ...
            ->setCellValue('L'.$n,strtoupper($row["PROVEEDOR"]))
            ->setCellValue('M'.$n,$row["FECHA_DE_INSTALACION"])
            ->setCellValue('N'.$n,$row["ESTADO"])
			->setCellValue('O'.$n,strtoupper($row["PROPIEDAD"]))
			->setCellValue('P'.$n,$row["PRECIO"])
			->setCellValue('Q'.$n,$row["FECHA_CREACION"])
            ->setCellValue('R'.$n,$row["GARANTIA"])
           ->setCellValue('S'.$n,strtoupper($row["COMPRA_POR"]))
			->setCellValue('T'.$n,strtoupper($row["CONVENIO_EVENTO"]))
			->setCellValue('U'.$n,strtoupper($row["PARTIDA"]))
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('InventarioHistorico');
